...
export default function AdminCoursesPage() {
  ...
  async function save() {
    ...
    const res = await fetch(isNew ? "/api/admin/courses" : `/api/admin/courses/${(modalCourse as Course).id}`, {
...
function LessonCard({
  ...
  async function togglePreview() {
    ...
    const res = await fetch(`/api/admin/lessons/${lesson.id}/preview`);
    const data = await res.json().catch(() => ({}));
...
function CourseLanguagesModal({
  ...
  async function saveField(languageId: string, field: "videoUrl" | "subtitleUrl" | "ebookUrl", storageKey: string | null) {
    ...
    const res = await fetch(`/api/admin/courses/${course.id}/languages/${languageId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  async function removeLanguage(languageId: string) {
    await fetch(`/api/admin/courses/${course.id}/languages/${languageId}`, { method: "DELETE" });
