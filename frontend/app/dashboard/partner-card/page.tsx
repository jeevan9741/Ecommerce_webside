...
import type { PartnerCardView } from "@/lib/partner-card/types";
...
export default async function PartnerCardPage() {
  const { defaults, ...view } = await serverApi<PartnerCardView & { defaults: PartnerCardDefaults }>("/partner-card");
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <PartnerCardManager initial={view} defaults={defaults} />
    </div>
  );
}
