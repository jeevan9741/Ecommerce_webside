...
export default function AdminWithdrawalsPage() {
  ...
  async function load() {
    const data = await backendFetch("/api/admin/withdrawals").then((r) => r.json());
    setWithdrawals(data.withdrawals);
  }

  useEffect(() => {
    backendFetch("/api/admin/withdrawals")
      .then((r) => r.json())
  ...
  async function process(id: string, action: "SUCCESS" | "FAILED" | "PROCESSING") {
    setBusyId(id);
    await backendFetch(`/api/admin/withdrawals/${id}/process`, {
      method: "POST",
