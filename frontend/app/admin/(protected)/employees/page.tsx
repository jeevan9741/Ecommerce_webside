...
export default function AdminEmployeesPage() {
  ...
  async function load() {
    const data = await backendFetch("/api/admin/employees").then((r) => r.json());
    setEmployees(data.employees);
  }

  useEffect(() => {
    backendFetch("/api/admin/employees")
      .then((r) => r.json())
  ...
  async function save() {
    ...
    const res = await backendFetch(isNew ? "/api/admin/employees" : `/api/admin/employees/${(modal as Employee).id}`, {
  ...
  async function remove(id: string) {
    await backendFetch(`/api/admin/employees/${id}`, { method: "DELETE" });
    await load();
  }
